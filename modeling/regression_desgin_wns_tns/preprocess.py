...
def load_data(flag):
    cmd = 'en'
    # cmd = 'sog'
    dc_label_data = f"./data/feat_label/label_{flag}.pkl"
    feat_data = f'./data/feat_label/feat_{flag}_{cmd}.pkl'
    std = StandardScaler()
    ...
    if flag == 'tns':
        pass
        # df2.drop(df2.columns[[2,3,4,5,6,7,8,9]], axis=1, inplace=True) ### for tns
    else:
        df2.drop(df2.columns[[0,2,3,4,5,6,7]], axis=1, inplace=True) ### for wns
...
def draw_fig_kf(title, y_pred, y_test, method, train_test):
    ...
    plt.text( 0.05, 0.75,  fontdict={'family':'Times New Roman'}, s=f'R2 = {r2}\nMAPE = {mape_val}%\nRMSE = {rmse}', 
